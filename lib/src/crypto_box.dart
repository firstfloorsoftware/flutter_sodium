...
class CryptoBox {
  ...
  static Future<Uint8List> encryptBytes(
          Uint8List value, Uint8List nonce, Uint8List publicKey, Uint8List secretKey) =>
      Sodium.cryptoBoxEasy(value, nonce, publicKey, secretKey);

  /// Verifies and decrypts a cipher text produced by encrypt.
  static Future<String> decrypt(
      Uint8List cipherText, Uint8List nonce, Uint8List publicKey, Uint8List secretKey) async {
    final message =
  ...
  static Future<Uint8List> decryptBytes(
          Uint8List cipherText, Uint8List nonce, Uint8List publicKey, Uint8List secretKey) =>
  ...
  static Future<DetachedCipher> encryptDetached(
      String value, Uint8List nonce, Uint8List publicKey, Uint8List secretKey) async {
    var map =
        await Sodium.cryptoBoxDetached(utf8.encode(value), nonce, publicKey, secretKey);
  ...
  static Future<DetachedCipher> encryptBytesDetached(
      Uint8List value, Uint8List nonce, Uint8List publicKey, Uint8List secretKey) async {
    var map = await Sodium.cryptoBoxDetached(value, nonce, publicKey, secretKey);
    return DetachedCipher.fromMap(map);
  }

  /// Verifies and decrypts a detached cipher text and tag.
  static Future<String> decryptDetached(
      DetachedCipher cipher, Uint8List nonce, Uint8List publicKey, Uint8List secretKey) async {
  ...
  static Future<Uint8List> decryptBytesDetached(
          DetachedCipher cipher, Uint8List nonce, Uint8List publicKey, Uint8List secretKey) =>
      Sodium.cryptoBoxOpenDetached(cipher.cipher, cipher.mac, nonce, publicKey, secretKey);
}